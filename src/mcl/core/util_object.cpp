...
{
	double x = luaL_checknumber(L, 1); double y = luaL_checknumber(L, 2); double z = luaL_checknumber(L, 3);
	...
	double m20 = -cx * sy; double m21 = sx; double m22 = cx * cy;

	double tx = 0.0, ty = 0.0, tz = 0.0;
	if (m20 < 1.0) {
		if (m20 > -1.0) { ty = std::asin(m20); tz = std::atan2(m10, m00); tx = std::atan2(m21, m22); }
		else { ty = -NINETY_DEG; tz = -std::atan2(-m12, m11); tx = 0.0; }
	} else { ty = NINETY_DEG; tz = std::atan2(-m12, m11); tx = 0.0; }

	lua_settop(L, 1);
	lua_pushnumber(L, tx); lua_pushnumber(L, ty); lua_pushnumber(L, tz);
	return 3;
}
