...
#include <cmath>
#include <lua.hpp>
#include <chrono>  // 精密な時間計測のため追加
#include <thread>  // スレッド停止（スリープ）のため追加

namespace util_shape {
/*
	// ─── 【新設：動的ウェイトレジスタ】 ───
	//     チャットコマンドから注入されたミリ秒単位のウェイト時間をここに常駐キャッシュ。
	//     初期値は 0（スリープなしの最速状態）。
	static int var_aabb_weight_ms = 0;

	inline int bisect_cpp(const std::vector<double>& vec, double val) {
		auto it = std::lower_bound(vec.begin(), vec.end(), val);
		return std::distance(vec.begin(), it) + 1;
	}

	inline void mark_occupied_cpp(std::vector<uint32_t>& solids, int b_disp, int x, int y, int z) {
		int index = (x << (b_disp + b_disp)) + (y << b_disp) + z;
		int word_idx = index / 32;
		int bit_idx = index % 32;
		if (word_idx >= 0 && word_idx < (int)solids.size()) {
			solids[word_idx] |= (1U << bit_idx);
		}
	}
*/
...
/*
		// ─── 【水際スピードリミッター起爆】 ───
		//     もしチャットからウェイト（例: 5ms）が指定されていた場合、
		//     この関数の実務が動く直前に、指定ミリ秒分だけ正確にスレッドを安全に一時停止させる！
		if (var_aabb_weight_ms > 0) {
			std::this_thread::sleep_for(std::chrono::milliseconds(var_aabb_weight_ms));
		}
		// ─── リミッターここまで ───
*/
		int target_idx = 1;
		...
		// ─── 🏆 【成果物オブジェクトへの遺伝子結合（大開通）】 ───
		//     C++側が新しく創世したプレーンテーブル（rgn_idx）に対し、
		//     Lua側のオリジナルのメタテーブル（クラス定義）を強制結合させて出荷する！
		luaL_getmetatable(L, "mcl_core_map_object"); // エンジンが保持している幾何学クラスを引き抜く
		if (lua_istable(L, -1)) {
			lua_setmetatable(L, rgn_idx); // 成果物テーブルへガチッとインジェクション！
		} else {
			lua_pop(L, 1); // 存在しない場合はスタックをお掃除
		}
		// ─── 🏆 ───
...
/*
	// l_var_set_aabb_weight（Luaから数値を受け取ってC++の静的変数へセットする関数）
	int l_var_set_aabb_weight(lua_State *L) {
		var_aabb_weight_ms = luaL_checkinteger(L, 1);
		return 0;
	}
*/
} // namespace util_shape
