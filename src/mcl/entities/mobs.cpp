...
namespace mobs {
	...
	// 👑 1. 【native_update_mob_timers】：★本家と100%同期するフラグ保持規律へ完全修正！
	int l_native_update_mob_timers(lua_State *L)
	{
		...
		while (lua_next(L, -2) != 0) {
			if (lua_isstring(L, -2) && lua_isnumber(L, -1)) {
				std::string k_str = lua_tostring(L, -2);
				double v_num = lua_tonumber(L, -1) - dtime;
				
				// 🎯【絶対正義の境界防犯】：
				//    減算結果が 0 未満のマイナス宇宙へ置き去りにされるのを物理的に完全阻止！！！
				//    0.0 でピタッとクリップホールドさせることで、本家 check_timer の遅延リセット規律と100%完全対称結合！
				if (v_num < 0.0) {
					v_num = 0.0;
				}
				
				updates.push_back({k_str, v_num});
			}
			lua_pop(L, 1); 
		}
